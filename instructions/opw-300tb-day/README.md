...
|   OTel, Splunk)  |   NLB    |  100-200 pods  |          |   Elasticsearch) |
+------------------+          +----------------+          +------------------+
                              3.5 vCPU / 7 GiB each
                              55 GiB disk buffer
```
...
- **No single instance should process more than 33% of total volume** for HA during node failure. At 100 minReplicas, each pod handles 1% - well within bounds. ([Source](https://docs.datadoghq.com/observability_pipelines/scaling_and_performance/best_practices_for_scaling_observability_pipelines/))
...
At 300 TB/day, decentralized or hybrid is strongly recommended. Centralized introduces a single point of failure at unacceptable blast radius.
...
Blanket scanning with 40 rules consumes 83% of the throughput available to a no-SDS pipeline. A pipeline that processes 300 TB/day at the Basic tier would need over 6x the compute to handle the same volume with 40-rule blanket SDS.
...
| **300 TB/day** | **50 x 4 vCPU** | **100 x 4 vCPU** | **150 x 4 vCPU** |

**If your pipeline uses blanket SDS scanning,** multiply the Heavy + SDS pod count by approximately 2.5x.

### 300 TB/day Capacity Math

300 TB/day is the **total daily volume**. Log traffic is not uniform: most environments see a 2x peak-to-trough ratio between business-hours peaks and off-hours steady state. Size the fleet for the **peak rate**, not the daily average.

Using the conservative 1 TB/vCPU/day baseline:

```
Daily volume: 300 TB/day
  Steady state rate (0.7x): 210 TB/day = 2,431 MB/s
  Peak rate (2x steady):    420 TB/day = 4,861 MB/s

  The fleet runs at steady state ~57% of the day and peak ~43% of the day.
  (0.57 x 210) + (0.43 x 420) = 120 + 180 = 300 TB/day total.
...
Node pool requirements (2 pods per 8-vCPU node, 2 x 3.5 = 7.0 vCPU + 1.0 for OS/kubelet):
  At 200 pods on c7i.2xlarge (8 vCPU, 2 pods/node): 100 nodes
  At 200 pods on c7i.4xlarge (16 vCPU, 4 pods/node): 50 nodes
```
...
- For environments requiring Guaranteed QoS on memory, set `limits.memory` equal to `requests.memory`
...
|---|---|---|
| minReplicas | 100 | 350 vCPU; handles 210 TB/day steady state at 60% util |
| maxReplicas | 200 | 700 vCPU; handles 420 TB/day peak at 60% util |
| targetCPU | 60% | Scale early; 67% burst headroom per pod |
| scaleUp stabilization | 60s | React within 1-2 min |
| scaleUp policy | +50% or +50 pods/min (Max) | Fast proportional growth |
...
```
T+0 min:  100 pods (350 vCPU)  - Peak traffic ramps. CPU rises above 60%.
                                  100 pods can handle 350 TB/day at 100% - HPA begins scaling.
T+1 min:  150 pods (525 vCPU)  - HPA adds 50 pods. Exceeds peak demand at 60% target.
T+2 min:  200 pods (700 vCPU)  - HPA adds 50 pods, capped at max. Full headroom restored.
```
...
At scale, even small efficiency improvements from better autoscaling have real cost impact. A fleet of 200 OPW pods on c7i.2xlarge instances (~$260/month each, shared 2 pods/node) where a 5% reduction in average pod count saves approximately $1,300/month (5 fewer nodes).
...
| `drop_newest` | New events silently dropped. No backpressure. | Dual-shipped sources where original is preserved elsewhere |
...
Daily traffic follows a diurnal pattern: ~57% of the day at steady state (210 TB/day rate), ~43% at peak (420 TB/day rate), averaging to 300 TB/day total. The peak transition is handled by three layers:

```
Layer 1: HIGH minReplicas
  100 pods at 60% steady-state utilization
  Each pod has ~67% burst headroom (60% -> 100%)
  100 pods at 100% = 350 vCPU; HPA adds 50 pods within 1 min to cover peak (420 TB/day)
...
```
T+0:00  Peak traffic ramps. 4,861 MB/s hitting 100 pods (capacity: 3,500 MB/s at 100%)
        Each pod: 48.6 MB/s demand vs 35 MB/s capacity
        Pods burst from 60% toward 100%. Excess causes brief 503s.
...
T+1:00  HPA stabilization window expires. Scaling decision: +50 pods.
        Fleet: 150 pods (capacity: 5,250 MB/s at 100%, 3,150 MB/s at 60%)
        Capacity EXCEEDS peak demand at 60% target. Senders begin draining.

T+2:00  +50 pods. Fleet: 200 pods (capacity: 7,000 MB/s)
        Full headroom restored. Upstream sender buffers draining.

T+3-5:  Sender buffers fully drained. System stable at 60% utilization.

        [Peak period: ~10 hours at 420 TB/day rate]

T+end:  Peak subsides. scaleDown begins after 900s stabilization.
        scaleDown removes ~10%/min. Fleet: 200 -> 180 -> 162 -> ...
        Back to 100 pods within ~45 min.
```
...
At production scale, splitting and horizontal scaling combine well: 4 pods x 4 vCPU with split 2x20 rules achieved 21.6 TB/day total (250 MB/s).
...
### The CPU Paradox: SDS and Autoscaling

CPU-based autoscaling can actively harm SDS-heavy pipelines. When SDS saturates, CPU drops because workers are blocked waiting for the scanner to drain, not because the pipeline has spare capacity.

**If you observe SDS utilization consistently above 0.9, replace CPU-based HPA with KEDA/DPA scaling on `pipelines.source_buffer_utilization_mean`.**
...
- **Topology spread:** Pods spread across AZs with `maxSkew: 2`. Remove if single-AZ cluster.
  - **Cost note:** Multi-AZ topology spread means cross-zone traffic. AWS charges ~$0.01/GB for cross-AZ within region. GCP does not charge. Azure varies. At 300 TB/day, this cost can be meaningful.
- **Pod disruption budget:** `maxUnavailable: 10%` - at 100 pods = 10 can be disrupted at once.
