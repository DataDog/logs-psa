...
def lambda_handler(event, context):
    ...
    try:
        ...
        body_str = gzip.GzipFile( None, "rb", fileobj= io.BytesIO( response[ "Body" ].read() ) ).read().decode( "utf-8" )
        buffer_ = []
        ...
        for line in body_str.splitlines() :
            ...
            json_[ "@path" ] = datetime.datetime.strptime( json_[ "date" ] , "%Y-%m-%dT%H:%M:%S.000Z" ).strftime( "dt=%Y%m%d/hour=%H/{}".format( archive_name ) )
